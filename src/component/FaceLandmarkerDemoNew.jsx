...
const DEFAULT_VALUE_PIPELINE = [0, 1, 2, 3, 4];
const FaceLandmarker = () => {
  ...
  useEffect(() => {
    if(!isLoadFirstPageCamera.current) {
      console.time('cameraLoad'); // Start timing
      ...
      navigator.mediaDevices.getUserMedia({
        ...
        videoRef.current.addEventListener("loadeddata", () => {
          ...
          console.timeEnd('cameraLoad'); // end timing
  ...
  const disableCam = () => {
    // if (cameraRef.current) {
      ...
      // requestAnimationRef.current = false;
    // }
  };
  ...
  const drawBlendShapes = useCallback((el, blendShapes) => {
    ...
    if (!isLoadingRef.current && pipelineRef.current <= pipelineCount) {
      ...
      const eyeBlinkLeft = blendShapes[0]?.categories.find(
        (shape) => shape.categoryName === "eyeBlinkLeft"
      )?.score;

      const eyeBlinkRight = blendShapes[0]?.categories.find(
      ...
      } else if (eyeBlinkLeft > 0.65 && eyeBlinkRight > 0.65 && currentTask === 'kedip-mata') {
        isLoadingRef.current = true;
        setTimeout(() => {
          storeData(pipelineRef.current === pipelineCount).then((res) => {
            if (pipelineRef.current === pipelineCount) {
      ...
      } else if (eyeBlinkLeft < 0.2 && eyeBlinkRight < 0.2  && jawopenValue < 0.2 && currentTask === 'hadap-depan') {
        isLoadingRef.current = true;
        setTimeout(() => {
          storeData(pipelineRef.current === pipelineCount).then((res) => {
            if (pipelineRef.current === pipelineCount) {
      ...
      // else if (eyeBlinkLeft < 0.2 && eyeBlinkRight < 0.2  && jawopenValue < 0.2 && currentTask === 'face-active') {
      //   isLoadingRef.current = true;
      //   storeData(pipelineRef.current === pipelineCount).then((res) => {
      //     if (pipelineRef.current === pipelineCount) {
      //       disableCam()
